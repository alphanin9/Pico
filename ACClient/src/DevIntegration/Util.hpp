 ...
 */
pico::Bool IsProcess(pico::shared::HashedString aProcNameHash) noexcept;
 ...
 */
pico::Bool IsRust() noexcept;

/**
 * \brief Sleeps until the module is loaded.
 * \param aModuleName The hash of the module name to wait for.
 */
void WaitUntilModuleLoaded(pico::shared::HashedString aModuleName) noexcept;

// In case of several alternate module name selections (for instance, CS2 has both Vulkan and DX11 as render system choices, 
// if one is selected the other won't be loaded)
// Not terribly well optimized, but it doesn't really matter for something that'll run once and won't be used in production
 ...
 */
void WaitUntilOneOfModulesLoaded(const pico::Vector<pico::shared::HashedString>& aModuleNames) noexcept;
...
*/
void Die(pico::Uint32 aErrorCode) noexcept;
}