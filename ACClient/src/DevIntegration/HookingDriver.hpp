...
struct HookingDriver
{
    ...
    virtual pico::Bool Attach(uintptr_t aAddy, _In_ void* aDetour) noexcept = 0;
    ...
    /// @return true if hooking succeeded, false if it did not.
    virtual pico::Bool Attach(uintptr_t aAddy, _In_ void* aDetour, _Out_ void** aOriginal) noexcept = 0;

    /// @brief Detach from an already created hook.
    /// @param aAddy The address of the target function.
    /// @return true if the hook was able to be destroyed, false if it was not.
    virtual pico::Bool Detach(uintptr_t aAddy) noexcept = 0;
     ...
     */
    virtual pico::Size GetPatchSize(uintptr_t aAddy) noexcept = 0;
    ...
    static HookingDriver* GetDefault() noexcept;
};
}
