...
// (cons: JIT is evil for our purpose)
...
{
    static const auto s_func = LI_FN(NtQuerySystemInformation).in(LI_MODULE("ntdll.dll").cached());
...
NTSTATUS __stdcall Windows::NtQueryInformationProcess(HANDLE aHandle, Windows::PROCESSINFOCLASS aClass,
                                                      ...
                                                      _Out_ pico::Uint32& aSizeWritten) noexcept
{
    static const auto s_func = LI_FN(NtQueryInformationProcess).in(LI_MODULE("ntdll.dll").cached());
...
NTSTATUS __stdcall Windows::NtQueryInformationThread(HANDLE aHandle, Windows::THREADINFOCLASS aClass,
                                                     ...
                                                     _Out_ pico::Uint32& aSizeWritten) noexcept
{
    static const auto s_func = LI_FN(NtQueryInformationThread).in(LI_MODULE("ntdll.dll").cached());
...
NTSTATUS __stdcall Windows::NtQueryVirtualMemory(HANDLE aHandle, uintptr_t aBaseAddress,
                                                 ...
                                                 _Out_ pico::Uint32& aSizeWritten) noexcept
{
    static const auto s_func = LI_FN(NtQueryVirtualMemory).in(LI_MODULE("ntdll.dll").cached());
...
{
    static const auto s_func = LI_FN(NtAllocateVirtualMemory).in(LI_MODULE("ntdll.dll").cached());
...
{
    static const auto s_func = LI_FN(NtFreeVirtualMemory).in(LI_MODULE("ntdll.dll").cached());
...
{
    static const auto s_func = LI_FN(NtProtectVirtualMemory).in(LI_MODULE("ntdll.dll").cached());
...
NTSTATUS __stdcall Windows::NtCreateFile(_Out_ HANDLE& aFileHandle, ACCESS_MASK aDesiredAccess,
                                         _In_ Windows::OBJECT_ATTRIBUTES* aObjAttributes, _In_ Windows::IO_STATUS_BLOCK* aStatusBlock,
...
{
    static const auto s_func = LI_FN(NtCreateFile).in(LI_MODULE("ntdll.dll").cached());
...
{
    static const auto s_func = LI_FN(NtReadVirtualMemory).in(LI_MODULE("ntdll.dll").cached());

    return s_func(aProcessHandle, aBaseAddress, aBuffer, aSize, aSizeRead);
}

void __stdcall Windows::RtlInitUnicodeString(_Out_ Windows::UNICODE_STRING& aStr, _In_ const pico::WChar* aSource) noexcept
{
    static const auto s_func = LI_FN(RtlInitUnicodeString).in(LI_MODULE("ntdll.dll").cached());
...
{
    static const auto s_func = LI_FN(NtDuplicateObject).in(LI_MODULE("ntdll.dll").cached());
...
{
    static const auto s_func = LI_FN(NtQueryObject).in(LI_MODULE("ntdll.dll").cached());
