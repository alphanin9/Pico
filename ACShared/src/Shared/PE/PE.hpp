 ...
 * failure.
 */
pico::shared::PE::Image* GetImagePtr(_In_ void* const aPtrInImage);
 ...
 */
pico::Bool IsImageValid(_In_ const pico::shared::PE::Image* const aImage);
 ...
 */
std::pair<uintptr_t, uintptr_t> GetImageBounds(_In_ const pico::shared::PE::Image* const aImage);

/**
 * \brief Gathers all the function entries from the image's exception information directory and returns their bounds as
 * RVAs.
 *
 * \param aImage A pointer to an image structure the data is queried from.
 *
 ...
 */
pico::Bool IsIntegrityCheckableImageSizeSmall(_In_ const pico::shared::PE::Image* const aImage);
 ...
 */
pico::Bool HasLargeRWXSections(_In_ const pico::shared::PE::Image* const aImage);
 ...
 * \param aImage A pointer to an image structure the data is queried from.
 *
 * \return The path to the program debug database file. If the information is not present or the function fails, returns
 * an empty string.
 */
pico::String GetImagePDBPath(_In_ const pico::shared::PE::Image* const aImage);
 ...
 * \param aImage A pointer to an image structure the data is queried from.
 *
 * \return A vector containing every RVA of an IMAGE_REL_BASED_DIR64 typed relocation in the image. If the function
 * fails, returns an empty vector.
 */
pico::Vector<pico::Uint32> GetRelocations(_In_ const pico::shared::PE::Image* const aImage);
} // namespace pico::shared::PE